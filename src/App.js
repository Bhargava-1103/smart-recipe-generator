...
export default function App() {
  ...
  const generate = () => {
    ...
    const res = data.filter((r) => {
      const ingMatch =
        userIng.length === 0 ||
        r.ingredients.every((i) => userIng.includes(i));
  ...
  return (
    <div className="container">
      ...
      <div className="controls">
        ...
        {/* ✅ IMAGE UPLOAD – SIMULATED INGREDIENT DETECTION */}
        <input
          type="file"
          accept="image/*"
          onChange={() => {
            alert(
              "Image uploaded. Ingredients detected successfully (demo)."
            );
            setIngredients("rice, vegetables");
          }}
        />
      ...
      {!loading &&
        recipes.map((r) => (
          <div key={r.id} className="card">
            ...
            <p>
              Calories: {r.calories * serving} | Protein: {r.protein}
            </p>

            <p className="small">
              Substitutions: soy sauce → tamari, paneer → tofu
            </p>
